...
?>
        ...
        <?php 
		 ...
		 {
            $stud_parents_id = $_GET['parent_id'];
		?>

        <input type="hidden" name="stud_parents_id" id="stud_parents_id" value="<?php echo $stud_parents_id ?> ">
          ...
          <div class="col-sm-10 col-sm-offset-1">
               <div class="panel panel-success">
                        <div class="panel-heading">
                           <?php echo ($action=="add")? "Isikan Maklumat Ibubapa/Penjaga": "Maklumat Ibubapa/Penjaga"; ?>
                        </div>
    ...
    <script src="jAlert-master/dist/jAlert-functions.min.js"> //optional!!</script>
         <script type="text/javascript">
             ...
             function update_info_parent()
             {
                var formData = $('#formParent').serialize();
               // event.preventDefault();
               // var fatherselectedValue = document.getElementById('father_name').value;
                

                //if (selectedValue) {
                    $.ajax({
                        url: 'update_parent.php',
                        type: 'POST',
                        data: formData,
                     ...
                     //$('#formParent').find(':input').prop('disabled', true);
         ...
         <script type="text/javascript">
            function carian(value)
            {
               
               // var selectedValue = document.getElementById('stud_parents_id').value;
               ...
               if (selectedValue) {
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            document.getElementById('sname').focus();
                            $('#fieldStudent').find(':input').prop('disabled', true);
         ...
         <script>
                $(document).ready(function() {
                   // $('#year_id').on('change', function() {
                        ...
                        if (selectedValue) {
                            $.ajax({
                                url: 'get_info_parents.php',
                                type: 'POST',
                                data: { parents_id: selectedValue },
                                success: function(response) {
                                    // Insert the received content into the contentContainer div
                                    $('#contentParents').html(response);
                                }
                            });

                            $.ajax({
                                url: 'get_list_students.php',
                                type: 'POST',
                                data: { parents_id: selectedValue },
                                success: function(response) {
                                    // Insert the received content into the contentContainer div
                                    $('#contentStudents').html(response);
                                }
                            });

                            $.ajax({
                                url: 'get_list_studentsfees.php',
                                type: 'POST',
                                data: { parents_id: selectedValue },
                                success: function(response) {
                                    // Insert the received content into the contentContainer div
                                    $('#contentStudentsFees').html(response);
                                }
                            });
